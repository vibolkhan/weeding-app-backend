...
router.get("/guests", async (req, res) => {
  const pool = getPool();
  const [rows] = await pool.query(
    "SELECT id, name, status, code FROM guests ORDER BY id DESC"
  );
  res.json(rows);
});
...
router.post("/guests", async (req, res) => {
  ...
  res
    .status(201)
    .json({ id: result.insertId, name, status: status || "invited", code });
});
...
router.delete("/guests/:id", async (req, res) => {
  const pool = getPool();
  const [result] = await pool.query("DELETE FROM guests WHERE id = ?", [
    req.params.id,
  ]);
  if (!result.affectedRows) return res.status(404).json({ error: "Not found" });
  res.json({ ok: true });
