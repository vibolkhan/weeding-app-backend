...
router.get('/guests/by-code/:code', async (req, res) => {
  ...
  const pool = getPool();
  const [rows] = await pool.query('SELECT id, name, status FROM guests WHERE code = ?', [code]);
  if (!rows.length) return res.status(404).json({ error: 'Guest not found' });
  const g = rows[0];
  res.json({ id: g.id, name: g.name, status: g.status });
});

// Admin list
router.get('/guests', requireAuth, async (req, res) => {
  const pool = getPool();
  const [rows] = await pool.query('SELECT id, name, status, code, created_at FROM guests ORDER BY created_at DESC');
  res.json(rows);
});
...
router.post('/guests', requireAuth, async (req, res) => {
  ...
  const code = genCode();
  const pool = getPool();
  const [result] = await pool.query('INSERT INTO guests (name, status, code) VALUES (?, ?, ?)', [name, status, code]);
